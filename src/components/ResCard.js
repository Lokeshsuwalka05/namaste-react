...
const ResCard = ({ resObj }) => {
  ...
  return (
    <div
      className="max-w-fit h-[330px] bg-white  border border-gray-200 rounded-lg shadow-sm p-3 hover:bg-gray-100 font-light hover:border-2"
      data-testid="ResCard"
    >
  ...
  return ({ resObj }) => {
    ...
    return (
      <div className="relative">
        <label className="absolute font-extrabold text-white top-44 left-8">
          {header + " " + subHeader}
        </label>
        <ResCard {...{ resObj }} />
      </div>
