  ...
  return (
    <div className="w-6/12 m-auto">
      ...
      <div className="ResMenu">
        <h2 className="flex justify-center font-serif text-gray-500 m-3 p-3 tracking-[0.3em]">MENU</h2>

        {cards.filter(
          (c) =>
            c?.card?.card["@type"] ===
            "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory"
        ).map((c,index)=><MenuCategory key={index} category={c.card.card}></MenuCategory>)}
      </div>
    </div>
  );
};
